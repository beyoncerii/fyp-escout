...
class EventController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        // Check if there are any athletes with status 'pending' or 'accepted' for this event
        $hasPendingOrAcceptedAthletes = Activity::where('event_id', $event->id)
            ->whereIn('status', ['pending', 'accepted'])
            ->exists();

        return view('editevent', compact('event', 'sports', 'hasPendingOrAcceptedAthletes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Add sport_id validation rule only if there are no pending or accepted athletes
        if (!$hasPendingOrAcceptedAthletes) {
        ...
        $event->update([
            ...
            'sport_id' => $validated['sport_id'] ?? $event->sport_id, // Only update sport_id if it's not null
        ]);
    ...
    public function pickAthletes(Request $request, Event $event)
    {
        ...
        foreach ($athletes as $athlete) {
            // Ensure both $event and $athlete are passed to the Mailable
            Mail::to($athlete->email)->send(new AthleteScouted($event, $athlete));
        }

        // Redirect to the view page to show the updated list of scouted athletes
        return redirect()->route('events.view', $event->id)->with('success', 'Athletes have been picked and notified.');
    }
